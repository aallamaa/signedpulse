...
# server.toml
[command]
argv = ["/usr/local/sbin/signedpulse-hook", "{ip}", "{client_id}"]
```

```sh
# signedpulse-hook (nftables): allow the verified IP through
nft add element inet filter signedpulse_allow "{ $1 }"
```

The client can even choose *which* port to open by emitting it from
`param_command` and passing `{param}` to the hook (validate it against an
allow-list — see `examples/signedpulse-hook.sh`). Pair the open pinhole with a
short timeout or a periodic reconciler if you want it to auto-close.
...
counters:  hello=144 verified=140 rejected=4 replays=0
clients:
...
[command]
# Placeholders substituted as literal arguments: {ip} {client_id} {source_port} {param}
